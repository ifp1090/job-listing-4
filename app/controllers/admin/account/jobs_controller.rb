class Admin::Account::JobsController < ApplicationController
  ...
  def index
    @jobs = current_user.jobs.recent.paginate(:page => params[:page], :per_page => 15)
  end
  ...
  def show
    ...
    if @job.is_hidden
      redirect_to admin_jobs_path, alert: "这个职位是隐藏的，不能观看哦"
    end
  end
  ...
  def update
    ...
    if @job.update(job_params)
      redirect_to admin_jobs_path, notice: "职位更新成功！"
    else
      render :edit
    end
  end
